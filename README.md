...
`tracefunc` takes a function and its arguments, executes it, and returns a `TraceResults` list of per-call traces. Each entry is `(stack_str, trace_dict)` where `stack_str` is the call stack (filtered so `fn` is the shallowest frame shown; empty when `target_func` is `None`) and `trace_dict` maps AST-level snippets to `(hit_count, vars_map)` with per-hit samples. Comprehensions show up as their own lines with per-iteration values. Lines that never executed are omitted unless `incl_unhit=True`, and if the function raises, the exception lands in the result’s `exc` attribute with the traces gathered up to the raise still returned.
