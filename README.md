...
`tracefunc` executes a Python function and records which statements ran, how often they ran, and the variable values observed at each hit. It traces AST-level snippets, including separate statements on one physical line and per-iteration values in comprehensions.

The result is a `TraceResults` list of per-call traces. Each entry contains `(stack_str, trace_dict)`:

- `stack_str` shows the call stack with `fn` as the shallowest displayed frame. It is empty when `target_func` is `None`.
- `trace_dict` maps source snippets to `(hit_count, vars_map)`, including samples for individual hits.

Unexecuted statements are omitted unless `incl_unhit=True`. If the function raises, the result still contains the traces collected so far, and its `exc` attribute holds the exception.
